...
export default function Login() {
  ...
  const handleLogin = () => {
    if (!login(email, password)) setError("Invalid email. Try alex@example.com");
    else {
      hapticSuccess();
      router.replace("/(tabs)");
    }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1">
        <ScrollView contentContainerClassName="px-6 py-8 gap-4" keyboardShouldPersistTaps="handled">
          <View className="items-center mb-4 mt-4">
            <View className="w-14 h-14 rounded-2xl bg-savr-600 items-center justify-center mb-3">
              <Text className="text-white text-xl font-bold">S</Text>
            </View>
            <Text className="text-3xl font-bold text-savr-900 dark:text-savr-100">Welcome back</Text>

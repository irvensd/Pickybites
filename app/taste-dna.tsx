  ...
  return (
    <View>
      <View className="flex-row justify-between mb-1">
        <Text className={`text-sm ${ui.text.secondary}`}>{label}</Text>
        <Text className={`text-sm font-semibold ${ui.text.primary}`}>{score}%</Text>
      </View>
      <View className={`h-2 rounded-full overflow-hidden ${ui.surface.track}`}>
        <View className="h-full bg-savr-600 rounded-full" style={{ width: `${score}%` }} />
      </View>
    </View>
  );
}
  ...
  return (
    <ScrollView className={`flex-1 ${ui.screen}`} contentContainerClassName="px-4 pb-6 gap-4">
      <Card className="items-center py-6">
        ...
        <Text className={`text-lg font-bold mt-2 ${ui.text.primary}`}>Your Flavor Profile</Text>
        <Text className="text-3xl font-bold text-savr-600 dark:text-savr-400 mt-2">{dna.averageRating.toFixed(1)}</Text>
        <Text className={`text-xs ${ui.text.muted}`}>Average Rating</Text>
      </Card>
      ...
      {dna.mostReviewedCuisine && (
        <Card>
          <Text className={`text-sm ${ui.text.secondary}`}>Most Reviewed</Text>
          <Text className={`font-semibold ${ui.text.primary}`}>{dna.mostReviewedCuisine}</Text>
        </Card>
      )}
      {dna.preferredPriceLevel && (
        <Card>
          <Text className={`text-sm ${ui.text.secondary}`}>Preferred Price</Text>
          <Text className={`font-semibold ${ui.text.primary}`}>{formatPrice(dna.preferredPriceLevel)}</Text>
        </Card>
      )}
      {dna.favoriteCuisines.length > 0 && (
        <View className="gap-2">
          <Text className={`font-semibold ${ui.text.primary}`}>Favorite Cuisines</Text>
          {dna.favoriteCuisines.slice(0, 5).map((c) => (
            <Card key={c.cuisine} className="flex-row justify-between items-center">
              <Text className={`font-medium ${ui.text.primary}`}>{c.cuisine}</Text>
      ...
      {dna.topDishes.length > 0 && (
        <View className="gap-2">
          <Text className={`font-semibold ${ui.text.primary}`}>Top Dishes</Text>
          {dna.topDishes.map((d) => {
